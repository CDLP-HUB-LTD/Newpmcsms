...
/// Where the voice message is coming from.
enum _MessageSource { recentMessage, uploadMp3 }

/// Which bulk-upload template the user picked.
enum _UploadTemplate { template1, template2 }

/// Step within the upload-template wizard.
enum _UploadStep { chooseTemplate, uploadFile, voiceMessage, timing }
...
class _VoiceSmsViewState extends ConsumerState<VoiceSmsView> {
  ...
  // Voice-message step (Template 2 only)
  PlatformFile? _uploadedVoiceFile;
  bool _isPickingVoiceFile = false;
  _MessageSource _uploadVoiceSource = _MessageSource.recentMessage;
  ...
  void _resetUploadFlow() {
    ...
    _selectedUploadTemplate = null;
    _uploadedFile = null;
    _uploadedVoiceFile = null;
    _uploadVoiceSource = _MessageSource.recentMessage;
  ...
  Widget _buildUploadTab() {
    switch (_uploadStep) {
      ...
      case _UploadStep.voiceMessage:
        return _buildTemplateWizard(_buildVoiceMessageStep());
      case _UploadStep.timing:
      ...
      child: Column(
        ...
        children: [
          _templateCard(
            ...
            title: 'Template 1',
            description:
                'Upload file with sender ID(s), Recipients and an existing recorded message',
          ...
          _templateCard(
            ...
            title: 'Template 2',
            description:
                'Upload file with sender ID(s), Recipients and a new recorded or uploaded voice message',
            icon: Icons.description_outlined,
          ),
        ],
      ),
  ...
  // ── STEP 1: UPLOAD FILE (recipient list) ────────────────────────────────
  ...
  // ── STEP 2 (Template 2 only): VOICE MESSAGE ─────────────────────────────
  Future<void> _pickVoiceFile() async {
    setState(() => _isPickingVoiceFile = true);
    try {
      final result = await FilePicker.platform.pickFiles(
        type: FileType.custom,
      ...
      if (result != null && result.files.isNotEmpty) {
        setState(() => _uploadedVoiceFile = result.files.single);
      }
    } finally {
      if (mounted) setState(() => _isPickingVoiceFile = false);
    }
  }

  Widget _buildVoiceMessageStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Row(
          children: [
            _radioOption(
              ...
              selected: _uploadVoiceSource == _MessageSource.recentMessage,
              onTap: () => setState(
                  () => _uploadVoiceSource = _MessageSource.recentMessage),
            ),
            _radioOption(
              label: 'Upload mp3',
              selected: _uploadVoiceSource == _MessageSource.uploadMp3,
              onTap: () =>
                  setState(() => _uploadVoiceSource = _MessageSource.uploadMp3),
            ),
          ],
        ),
        const VerticalSpacing(12),
        if (_uploadedVoiceFile == null)
          Container(
            ...
            child: InkWell(
              onTap: _isPickingVoiceFile
                  ? null
                  : _uploadVoiceSource == _MessageSource.recentMessage
                      ? () {
                          // TODO: start/stop recording via a package like
                          // `record`, then setState(() => _uploadedVoiceFile
                          // = <recorded file>).
                        }
                      : _pickVoiceFile,
              child: _isPickingVoiceFile
                  ? const Center(
                      child: SizedBox(
                        height: 20,
                        width: 20,
            ...
            child: Row(
              children: [
                IconButton(
                  ...
                  onPressed: () {
                    // TODO: play/pause via an audio player package
                  },
                ),
                Expanded(
                  child: Text(
                    _uploadedVoiceFile!.name,
                    style: context.textTheme.s12w400,
                    overflow: TextOverflow.ellipsis,
                  ),
                ),
                InkWell(
                  onTap: () => setState(() => _uploadedVoiceFile = null),
                  child: Text(
                    'Delete',
                    style:
                        context.textTheme.s12w500.copyWith(color: Colors.red),
        ...
        SizedBox(
          ...
          height: 50.h,
          child: ElevatedButton(
            onPressed: _uploadedVoiceFile == null
                ? null
  ...
  Widget _buildMessageSourcePicker() {
    return Row(
      children: [
        _radioOption(
          label: 'Recent Message',
          selected: _messageSource == _MessageSource.recentMessage,
          onTap: () => setState(() {
        ...
        _radioOption(
          label: 'Upload mp3',
          selected: _messageSource == _MessageSource.uploadMp3,
          onTap: () => setState(() {
            _messageSource = _MessageSource.uploadMp3;
          }),
        ),
      ],
    );
          ...
          child: Row(
            children: [
              IconButton(
                icon: const Icon(Icons.play_arrow_rounded),
                onPressed: () {
                  // TODO: play/pause the recording via an audio player package
                },
              ),
              const Expanded(
                child: SizedBox(height: 24), // TODO: real waveform widget
              ),
              const SizedBox(width: 8),
              Text(
        ...
        InkWell(
          ...
          child: Row(
            children: [
              ...
              Text(
                'Remove',
                style: context.textTheme.s12w500.copyWith(color: Colors.red),
              ),
            ],
          ),
        ),
