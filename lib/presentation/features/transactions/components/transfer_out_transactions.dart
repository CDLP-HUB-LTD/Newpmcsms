...
class TransferOutTransactions extends ConsumerStatefulWidget {
  const TransferOutTransactions({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _TransferOutTransactionsState();
}

class _TransferOutTransactionsState
    extends ConsumerState<TransferOutTransactions> {
  ...
  /// Only outgoing transfers — "Transfer to ..."
  bool _isTransferOut(String purpose) {
    return purpose.toLowerCase().startsWith('transfer to');
  }

  List<dynamic> _applyFilters(List<dynamic> list) {
    return list.where((transaction) {
      final purpose = (transaction.purpose ?? '').toString();
      final matchesType = _isTransferOut(purpose);
